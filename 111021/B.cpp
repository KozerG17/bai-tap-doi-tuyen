...
int main() {
	...
	init = lim - init;
	auto ans = vector<str>(lim + 1, str(n, '.'));
	For (i, 0, n) {
		if (s[i] == '+') ans[init--][i] = '/';
		if (s[i] == '-') ans[++init][i] = '\\';
		if (s[i] == '=') ans[init][i]   = '_';
	...
	if (ans[lim] == str(n, '.')) ans.pop_back();
	if (ans[0] == str(n, '.')) ans.erase(ans.begin());
	for (str &x : ans) cout << x << '\n';
}
...
// ------------------------------------------                    |\__/,|   (`\
// | author: dlbm1302 | created: 11.10.2021 |                  _.|o o  |_   ) )
// -----------------------------------------------------------(((---(((------------